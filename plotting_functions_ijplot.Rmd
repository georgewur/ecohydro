...
add_xlabel(frac.p, text = "Distance starting from the North Sea (m)")
add_ylabel(frac.p, text = "Vegetation fractions (-)")
...
add_ylabel(wb.p,text = "Flux in m2/d" , fontsize = 12)
add_xlabel(wb.p,text = paste("Date start",first.day," Date end", last.day),fontsize = 12)
##creating the legend

legend = new_ijplot("l",xfixed=TRUE,yfixed=TRUE,fxleft = 0.85,bottommargin=0,leftmargin=0,
                     horaxisnumdiv = 0, veraxisnumdiv = 0, imposedmaxx = length(wb.types)+1)#, fybottom)
...
# Water balance at each node (x-axis) during the simulation(time selector)
