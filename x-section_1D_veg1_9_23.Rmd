...
The cross section start at the North Sea side continuing over the dune towards the polders in the direction of the village den Burg. ![Cross section West-East](x-sectionW-E.png) The length of the model is about 3 km and the depth extends from the surface till 15m below MSL in depth where one can find a clay deposit of the Drente/Gieten formation and is assumed impermeable. The elevations at the surface are based on AHN.
...
begin.date = 20100331 #March 31 2010
end.date  =  20110330 #365 #length(meteo$yearmonthday)
begin.day.nr = which(meteo$yearmonthday == begin.date)
end.day.nr = which(meteo$yearmonthday == end.date)
current.day = begin.day.nr
delta.t = 1.0 #day
