...
The drainage resistance is based on the local drainage system of the polders (Cdrain = 100 days) and the vertical resistance of the soil when there is runoff is 10 days (Crunoff)
...
for (i in 1:10)
{
  fig2 = add.lines(fig2,x = nodes, y = df.evp[i,], type = "scatter", mode = "lines")
}
fig2
...
4.  Spatial flux for bare soil
    ...
    3.  return $Ea$ to the model

5.  Run the model with the current vegetation and bare soil flux
...
# for (i in begin.day.nr : (end.day.nr - 1))
# {
#   #calculate Tpi with current ETp(t+1) and fi(t)
#   Tpi.f2 = Calc.Tpi(meteo$EV24[i],f2)
#   #calculate Tai.Tpi(t+1) with current Tpi(t)
#      #current node (20=> 950m) and water table i comes from array w.table
#   Tai.Tpi.f2 = Calc.Tai.Tpi(Tpi.f2,w.table[i,43]*1000,f2.ai,f2.zi)
#   dfi.dt.f2 = Calc.dfi.dt(Tai.Tpi.f2,f2,f0,f2.di)
#   f2 = f2 + dfi.dt.f2
#   f0 = f0 - dfi.dt.f2
#   f0.part = c(f0.part,f0)
#   f2.part = c(f2.part,f2)
# }
...
# Tpi.f1 = seq(0,0, length = length(nodes))
# Tpi.f1.fun = approxfun(nodes,Tpi.f1)
# Tpi.f2 = seq(0,0, length = length(nodes))
# Tpi.f2.fun = approxfun(nodes,Tpi.f2)
Ea.f0 = seq(0,0, length = length(nodes))
Ea.f0.fun = approxfun(nodes, Ea.f0)

calc.Tai.f1 = function(x,state)
{
  f1 = f1.fun(x) #fetch the current f1 for the current node at x
  Tpi.f1  = Calc.Tpi(evaptrns,f1) #to have a global Tpi.f2
  #calculate Tai.Tpi(t+1) with current Tpi(t)
  Tai.Tpi.f1 = Calc.Tai.Tpi(Tpi.f1,max(0,(ahn2.fun(x)-state))*1000,f1.ai,f1.zi)
  Tai.f1 = Tai.Tpi.f1 * evaptrns
  return(-Tai.f1)
}
...
{
  f2 = f2.fun(x)
  Tpi.f2  = Calc.Tpi(evaptrns,f2) #to have a global Tpi.f2
  #calculate Tai.Tpi(t+1) with current Tpi(t)
  Tai.Tpi.f2 = Calc.Tai.Tpi(Tpi.f2,max(0,(ahn2.fun(x)-state))*1000,f2.ai,f2.zi)
  Tai.f2 = Tai.Tpi.f2 * evaptrns
  return(-Tai.f2)
}
...
calc.Ea.f0 = function(x,state)
{
  Ea = Ea.f0.fun(x)
  Ep = evaptrns - Ea
#  Ea = Ep * 1/(1+max(0,(ahn2.fun(x)- state))*1000/f0.zE)^f0.aE
    Ea = Ep * 1/(1+max(0,(ahn2.fun(x)-state))*1000/f0.zE)^f0.aE
  return(-Ea)
}
...
begin.date = 20121221 #March 31 2010
end.date  =  20151219 #365 #length(meteo$yearmonthday)
begin.day.nr = which(meteo$yearmonthday == begin.date)
end.day.nr = which(meteo$yearmonthday == end.date)
current.day = begin.day.nr
delta.t = 1.0 #day
...
#create vectors for the partitioning
f0.frac = c()
f1.frac = c()
f2.frac = c()
...
while (current.day < end.day.nr)
{
  ...
  # for (i in 1:100)
  # {
  # control=solve.step(Tex.veg)$RMSM
  # cat(paste('RMSM :',control,"iteration : ", i,'\n'))
  #   if (control < 0.0001) break
  # if(i == 100)
  # {
  #   error.log = c(error.log, paste("No convergence at day: ", meteo$yearmonthday[current.day],
  #                                  "precip : ", meteo$RH[current.day], "evap : ", meteo$EV24[current.day]))
  #   do.initialize(Tex.veg, init = init.H.state)                  
  # }
  # }
  
  #print(paste("daynr",current.day))
  #saving some intermediate water balance data in wb.df
   ...
   wb = rbind(wb, c(meteo$yearmonthday[current.day],wb.df[2,3],wb.df[3,2],wb.df[3,3],wb.df[4,3],
                    wb.df[5,3],wb.df[6,3],wb.df[7,2],wb.df[8,4]))
  ...
  wtable = ahn2.fun(nodes)-df.state$state
  ...
  Tai.f1 = df.external$vegetation2
  df1.dt = Calc.dfi.dt(Tai.f1/Tpi.f1,f1,f0,f1.di)
  f1 = f1.fun(nodes) + df1.dt
  f1.fun = approxfun(nodes,f1)
  f1.frac = rbind(f1.frac,f1)
  Tpi.f2 = Calc.Tpi(-evaptrns,f2.fun(nodes))
  Tai.f2 = df.external$vegetation2
  df2.dt = Calc.dfi.dt(Tai.f2/Tpi.f2,f2,f0,f2.di)
